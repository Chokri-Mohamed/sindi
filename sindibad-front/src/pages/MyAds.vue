...
<q-toolbar-title>
  ...
  Sindibad
      ...
      <q-space />
      <q-btn flat label="Home" class="q-mr-md" href="/homea"/>

      <q-btn flat label="profile" class="q-mr-md" href="userget/2" />
    </q-app-bar>
...
<div class="home-page">
      ...
      <div class="card-container">
        <div v-for="(group, index) in groupedAdverts" :key="index" class="card-group">
          ...
          <q-btn flat label="modify" color="primary" @click="modify(advert)" />
          <q-btn flat label="delete" color="primary"  @click="deleteAdvert(advert)" />
        </q-card-actions>
...
export default {
  ...
  methods: {
    async deleteAdvert (advert){
      const response = await axios.delete('http://localhost:8000/individual/adverts/' + advert.id);
      console.log(response)
      this.fetchdata()
      
    },
    async modify (advert){
      this.$router.push('/modify_advert/'+advert.id)      
    },
    onEnterKeyPressed() {
      console.log(this.q)
    ...
    async fetchdata() {
      try {
        ...
        this.adverts = this.adverts.filter(element => element.userId == this.$route.params.id)
